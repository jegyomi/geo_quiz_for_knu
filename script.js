let mainMode = ""; 
let currentMode = ""; // 'learn_country', 'learn_all', 'test_country', 'test_all'
...
// 📌 1.5단계 진입 (학습 or 시험 저장)
function showSubMode(type) {
    mainMode = type; // 'learn' 또는 'test' 저장
    document.getElementById('step-mode').classList.remove('active');
    document.getElementById('step-sub-mode-title').innerText = type === 'learn' ? '📖 어떻게 학습할까요?' : '✍️ 어떻게 시험 볼까요?';
    document.getElementById('step-sub-mode').classList.add('active');
}

// 📌 1.5단계 완료 시 모드 결정 후 교수님 선택으로
function setSubMode(subType) {
    currentMode = mainMode + "_" + subType; // ex: 'learn_country', 'test_all' 등
...
function startGame() {
    ...
    let modeText = "";
    if (currentMode === 'learn_country') modeText = "학습 (국가)";
    if (currentMode === 'learn_all') modeText = "학습 (국가+수도)";
    if (currentMode === 'test_country') modeText = "시험 (국가)";
    if (currentMode === 'test_all') modeText = "시험 (국가+수도)";
    
    document.getElementById('game-title').innerText = `${regionNamesKR[currentRegion]} (${modeText})`;
    
    // 📌 UI 상태 업데이트 로직 개편
    if (currentMode.startsWith('learn')) {
        document.getElementById('input-group').style.display = 'none';
        document.getElementById('submit-btn').innerText = '정답 확인 (Enter)';
        document.getElementById('quiz-instruction').innerText = '📍 위치를 확인하고 Enter를 눌러 정답을 확인하세요!';
    } else {
        document.getElementById('input-group').style.display = 'block';
        document.getElementById('submit-btn').innerText = '정답 제출 (Enter)';
        
        // 시험 모드인데 '국가'만 본다면 수도칸 숨기기!
        if (currentMode === 'test_country') {
            document.getElementById('input-capital').style.display = 'none';
            document.getElementById('quiz-instruction').innerText = '📍 위치를 확인하고 국가명을 적어주세요!';
        } else {
            document.getElementById('input-capital').style.display = 'inline-block';
            document.getElementById('quiz-instruction').innerText = '📍 위치를 확인하고 국가와 수도를 적어주세요!';
        }
    }
...
function showNextQuestion() {
    ...
    document.getElementById('toast-popup').className = ""; 
...
function showPopup(msg, success, duration = 1200, customImg = null) {
    ...
    clearTimeout(popupTimeout);
    if (duration > 0) {
        popupTimeout = setTimeout(() => p.className = "", duration);
    }
}
...
function checkAnswer() {
    ...
    if (currentMode.startsWith('learn')) {
        if (!isAnswerRevealed) {
            ...
            let text = currentMode === 'learn_country' ? `📍 정답: ${q.country}` : `📍 정답: ${q.country} - ${q.capital}`;
            let img = profInfo && profInfo.img_correct ? profInfo.img_correct : null;
            
            if (!isMuted) {
                let sfx = new Audio("assets/learn_effect.mp3");
                sfx.volume = globalVolume;
                sfx.play().catch(e => {});
            }

            showPopup(text, true, 0, img); 
        } else {
    ...
    // 세부 모드에 따른 정답 인정 조건 다름
    if (currentMode === 'test_country') {
        isCorrect = (inCountry === q.country);
    } else { // test_all
        isCorrect = (inCountry === q.country && inCapital === q.capital);
    }
    ...
    if (isCorrect) {
        ...
        let correctText = currentMode === 'test_country' ? `[${q.country}]` : `[${q.country} - ${q.capital}]`;
        showPopup(`❌ 오답!\n정답은 ${correctText}`, false, 1500, wrongImg);
...
// 📌 엔터키 입력 똑똑하게 개편 (수도가 없으면 바로 제출)
document.getElementById('input-country').addEventListener('keypress', (e) => { 
    if (e.key === 'Enter') { 
        e.preventDefault(); 
        if (currentMode === 'test_country') {
            checkAnswer(); // 국가만 시험이면 바로 정답 제출!
        } else {
            document.getElementById('input-capital').focus(); // 아니면 수도창으로 이동
        }
    } 
});
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Enter' && document.getElementById('step-game').classList.contains('active')) {
        if (currentMode.startsWith('learn')) {
            e.preventDefault();
            checkAnswer();
        }
...
document.getElementById('map-inner').addEventListener('click', function(e) { 
    ...
    bgImg.onload = function() { 
        ...
        if (imgRatio > containerRatio) { renderW = rect.width; renderH = rect.width / imgRatio; offsetX = 0; offsetY = (rect.height - renderH) / 2; } 
        else { renderH = rect.height; renderW = rect.height * imgRatio; offsetX = (rect.width - renderW) / 2; offsetY = 0; } 
        const x = ((e.clientX - rect.left - offsetX) / renderW) * 100; 
        const y = ((e.clientY - rect.top - offsetY) / renderH) * 100; 
        if (x >= 0 && x <= 100 && y >= 0 && y <= 100) alert(`📍 오차 보정 완료 좌표!\n\nx: ${x.toFixed(2)}, y: ${y.toFixed(2)}\n\n(이 값을 복사해서 regionData에 넣어주세요)`); 
    }; 
});